...
def data_split(two_lines):
    ...
    if answer is not None: #getting rid of None lines
        ...
        #('DATE= (\d+\/\d+\/\d+) TIME= (\d+\:\d+\:\d+)_N_(.{9}_.{4})_(.{3})_(.{2})_(.{3})\s+CYCLE_TIME: (\d+) MIN, (\d+\.\d+) SEC')
        pat = re.compile('DATE= (\d+\/\d+\/\d+) TIME= (\d+\:\d+\:\d+)_N_(.{9}_.{4})_(.{3})_(.{2}_.{3})\s+CYCLE_TIME: (\d+) MIN, (\d+\.\d+) SEC')
        answer = re.search(pat, two_lines)
        if answer is not None:  # getting rid of None lines
            return answer.groups()
        #print(unified_line)
